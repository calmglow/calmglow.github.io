...
{
	ap_openwin("exec/egloo_delcomment_anony_viewa16a.html?eid=" + eid + "&srl=" + postserial + "&xhtml=" + xhtml + "&cmtserial=" + cmtserial + "&ismenu=" + ismenu, "delcomment", 400, 230, 3, false, false, false);
}
...
function cmtview(serial,eid,xhtml)
{
	...
	var cmtcont = "";
	var rtnurl = "";
	remoteURL = "egloo_commenta16a.html?eid=" + eid + "&srl=" + serial + "&xhtml=" + xhtml;
...
function tag_validate(obj) {
	...
	while ( (word = re.exec(obj.value)) != null ){
		...
		if ( tmpv.indexOf("index.html") != -1) {
			tmpv = tmpv.replace("index.html","");
			tagObj.add(tmpv,0,1);
		}
		else {
			tmpv = tmpv.replace("index.html","");
...
function go_search(form) {
    ...
    if(kwd) {
        ...
        var addurl = "search0f15.html?query=";
