...
PhotoCommentExec.prototype = {
	...
	replyComment: function (photoSerial,commentSerial){
		...
		{
			html += '<p class="re_cmt_name"><label for="re_name">이름</label> : <input type="text" id="re_name"  name="re_name" size="10" maxlength="10" value="' + this.userName + '" /></p><p class="re_cmt_passwd"><label for="re_passwd">비밀번호</label> : <input type="password" id="re_passwd" name="re_passwd" size="10" maxlength="10" value="" /></p><p class="re_cmt_homepage"><label for="re_homepage">블로그</label> : <input type="text" id="re_homepage" name="re_homepage" value="' + this.homePage + '" size="20" /></p>'; 
		}
	...
	deleteComment: function(eid, albumSerial, commentSerial, isAnonymous) {
		if(isAnonymous == false) {
			...
			ap_openwin("../exec/photo/photo_comment_anonya16a.html?eid=" + eid + "&srl=" + albumSerial + "&cmtsrl=" + commentSerial, "delcomment", 400, 300, 3, false, false, false);
		}
	}
}
